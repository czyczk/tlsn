...
use super::{OTSenderActor, TdnVerifierError};
...
pub(crate) struct MuxFuture {
    pub(crate) fut:
        Pin<Box<dyn FusedFuture<Output = Result<(), TdnVerifierError>> + Send + 'static>>,
}

impl Future for MuxFuture {
    type Output = Result<(), TdnVerifierError>;
...
pub(crate) struct OTFuture {
    pub(crate) fut: Pin<
        Box<dyn FusedFuture<Output = Result<OTSenderActor, TdnVerifierError>> + Send + 'static>,
    >,
}

impl Future for OTFuture {
    type Output = Result<OTSenderActor, TdnVerifierError>;
